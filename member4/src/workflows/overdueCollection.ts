...
import { OVERDUE_COLLECTION_CONTEXT } from '../prompts/scenarioPrompts';

export class OverdueCollectionWorkflow {
  ...
  async execute(rentalId: string, phoneNumber: string): Promise<any> {
    try {
      ...
      const lateFee: any = await member3Client.calculateLateFee(rentalId);
      ...
      const context = OVERDUE_COLLECTION_CONTEXT(rentalDetails, lateFee?.amount || lateFee || 0);
